...
def cached_torrent
  ...
  begin
    ...
    elsif(new_resource.torrent =~ /^(https?:\/\/)(.*\/)(.*torrent)$/)
      # if the file doesn't end in torrent, hash the url, and append .torrent
      cache_file_path = "#{Chef::Config[:file_cache_path]}/#{Digest::SHA1.hexdigest new_resource.torrent}.torrent"
      r = remote_file cache_file_path do
        source new_resource.torrent
        backup false
        mode "0755"
...
def torrent_hash
  ...
  # end
  Chef::Log.debug("#{cached_torrent} is about to be hashed... manually and everytime")
  Digest::SHA1.hexdigest((IO.read(cached_torrent).bdecode["info"]).bencode) # schisamo gives me headaches, thanks fujin
end
